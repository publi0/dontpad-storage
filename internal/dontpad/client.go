...
func (d *DontPad) UploadFile(content, key string) error {
	...
	req, err := http.NewRequest(http.MethodPost, host+key, data)
	if err != nil {
		return err
	}
	setHeaders(req)
...
func (d *DontPad) DownloadFile(key string) (string, error) {
	req, err := http.NewRequest(http.MethodGet, fmt.Sprintf("%s%s.body.json?lastModified=0", host, key), nil)
	if err != nil {
		return "", err
	}
	setHeaders(req)
